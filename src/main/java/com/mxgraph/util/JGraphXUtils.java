 ...
 */
public class JGraphXUtils
{

	private static final Logger log = Logger.getLogger(JGraphXUtils.class.getName());
			...
			mxRectangle vertexBounds, double scale, boolean isEdge)
	{
		...
		mxRectangle size = JGraphXUtils.getLabelSize(label, style, isHtml, scale,
				wrapWidth);
		...
		if (vertexBounds != null)
		{
					...
					mxConstants.SHAPE_SWIMLANE))
			{
				...
				double start = JGraphXUtils.getDouble(style,
		...
		return JGraphXUtils.getScaledLabelBounds(x, y, size, width, height, style,
				scale);
	}
	 ...
	 */
	public static int findNearestSegment(JGraphXCellState state, double x, double y)
	{
		int index = -1;
	 ...
	 */
	public static int getPortConstraints(JGraphXCellState terminal,
                                         JGraphXCellState edge, boolean source)
	{
	 ...
	 */
	public static int getPortConstraints(JGraphXCellState terminal,
                                         JGraphXCellState edge, boolean source, int defaultValue)
	{
	...
	public static boolean intersectsHotspot(JGraphXCellState state, int x, int y,
                                            double hotspot)
	{
                                            ...
                                            double hotspot, int min, int max)
	{
		if (hotspot > 0)
		{
			...
			{
				int start = JGraphXUtils.getInt(state.getStyle(),
						mxConstants.STYLE_STARTSIZE,
						mxConstants.DEFAULT_STARTSIZE);

				if (JGraphXUtils.isTrue(state.getStyle(),
						mxConstants.STYLE_HORIZONTAL, true))
				{
	...
	public static BufferedImage createBufferedImage(int w, int h,
			Color background)
	{
		return JGraphXUtils.createBufferedImage(w, h, background, (background != null) ? BufferedImage.TYPE_INT_RGB
	...
	public static BufferedImage loadImage(String url)
	{
		...
		if (url != null)
		{
			...
			else
			{
				...
				{
					realUrl = JGraphXUtils.class.getResource(url);
				}
			...
			int y, int w, int h, double scale, Map<String, Object> style)
	{
		...
		if (text != null && text.length() > 0)
		{
			...
			td.setAttribute("align", JGraphXUtils.getString(style,
			...
			if (JGraphXUtils.getString(style, mxConstants.STYLE_WHITE_SPACE,
					"nowrap").equals("wrap"))
			{
			...
			String text, double scale, int width, String head, String bodyCss)
	{
		...
		String color = JGraphXUtils.getString(style, mxConstants.STYLE_FONTCOLOR);
			...
			else
			{
				...
				if (attrs != null)
				{
					for (int i = 0; i < attrs.getLength(); i++)
					{
						...
						value = JGraphXUtils.htmlEntities(value);
