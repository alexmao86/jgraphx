 ...
 */
public class JGraphXComponent extends JScrollPane implements Printable
{

	private static final Logger log = Logger.getLogger(JGraphXComponent.class.getName());
	 ...
	 * 
	 */
	protected JGraphXInteractiveCanvas canvas;
	 ...
	 * @param graph
	 */
	public JGraphXComponent(JGraphX graph)
	{
	 ...
	 * 
	 */
	public void setGraph(JGraphX value)
	{
		JGraphX oldValue = graph;
	...
	public boolean isToggleEvent(MouseEvent event)
	{
		...
		return (event != null) ? ((JGraphXUtils.IS_MAC) ? ((SwingUtilities
	...
	public int print(Graphics g, PageFormat printFormat, int page)
	{
		...
		RepaintManager currentManager = RepaintManager
				.currentManager(JGraphXComponent.this);
	 ...
	 * 
	 */
	public JGraphXInteractiveCanvas getCanvas()
	{
		return canvas;
	}
	...
	public JGraphXInteractiveCanvas createCanvas()
	{
		...
		return new JGraphXInteractiveCanvas();
	}
	 ...
	 * 
	 */
	public void insertComponent(JGraphXCellState state, Component c)
	{
	 ...
	 * 
	 */
	public void updateComponent(JGraphXCellState state, Component c)
	{
	...
	protected Rectangle paintBackgroundPage(Graphics g)
	{
		...
		if (isPageVisible())
		{
			...
			{
				g.setColor(c);
				JGraphXUtils.fillClippedRect(g, 0, 0, getGraphControl().getWidth(),
			...
			{
				g.setColor(c);
				JGraphXUtils.fillClippedRect(g, x0 + w, y0 + 6, 6, h - 6);
				JGraphXUtils.fillClippedRect(g, x0 + 8, y0 + h, w - 2, 6);
			}
			...
			g.setColor(bg);
			JGraphXUtils.fillClippedRect(g, x0 + 1, y0 + 1, w, h);
	 ...
	 */
	public void redraw(JGraphXCellState state)
	{
		if (state != null)
		{
	...
	protected void createTripleBuffer(int width, int height)
	{
		try
		{
			...
			JGraphXUtils.setAntiAlias(tripleBufferGraphics, antiAlias, textAntiAlias);
	...
	public void repaintTripleBuffer(Rectangle dirty)
	{
		if (tripleBuffered && tripleBufferGraphics != null)
		{
			...
			JGraphXUtils.clearRect(tripleBufferGraphics, dirty, null);
	...
	public class mxGraphControl extends JComponent
	{
		 ...
		 * 
		 */
		public JGraphXComponent getGraphContainer()
		{
			return JGraphXComponent.this;
		}
		...
		public void paintComponent(Graphics g)
		{
			...
			else
			{
				...
				try
				{
					JGraphXUtils.setAntiAlias(g2, antiAlias, textAntiAlias);
					drawGraph(g2, true);
				}
				finally
				{
		 ...
		 * 
		 */
		protected boolean hitClip(Graphics2DCanvas canvas, JGraphXCellState state)
		{
		 ...
		 */
		protected Rectangle getExtendedCellBounds(JGraphXCellState state)
		{
			Rectangle rect = null;

			// Takes rotation into account
			double rotation = JGraphXUtils.getDouble(state.getStyle(),
					mxConstants.STYLE_ROTATION);
			mxRectangle tmp = JGraphXUtils.getBoundingBox(new mxRectangle(state),
					rotation);

			// Adds scaled stroke width
			int border = (int) Math
					.ceil(JGraphXUtils.getDouble(state.getStyle(),
							mxConstants.STYLE_STROKEWIDTH)
							* graph.getView().getScale()) + 1;
			tmp.grow(border);

			if (JGraphXUtils.isTrue(state.getStyle(), mxConstants.STYLE_SHADOW))
			{
			...
			MouseMotionListener
	{
		 ...
		 * 
		 */
		public mxMouseRedirector(JGraphXComponent graphComponent)
		{
