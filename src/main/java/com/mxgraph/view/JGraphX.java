 ...
 */
public class JGraphX extends mxEventSource
{

	private static final Logger log = Logger.getLogger(JGraphX.class.getName());
	 ...
	 */
	public JGraphX(Stylesheet stylesheet)
	{
			...
			Map<String, Object> style)
	{
		if (style != null)
		{
			String key = JGraphXUtils.getString(style, mxConstants.STYLE_IMAGE);
			...
			Object[] cells)
	{
		...
		{
			JGraphXCellState state = view.getState(cells[0]);
			Map<String, Object> style = (state != null) ? state.getStyle()
					: getCellStyle(cells[0]);

			if (style != null)
			{
				String value = (JGraphXUtils.isTrue(style, key, defaultValue)) ? "0"
						: "1";
			...
			Object[] cells)
	{
		...
		if (cells != null && cells.length > 0)
		{
			if (value == null)
			{
				...
				if (style != null)
				{
					int current = JGraphXUtils.getInt(style, key);
	...
	public Object[] orderCells(boolean back, Object[] cells)
	{
		if (cells == null)
		{
			cells = JGraphXUtils.sortCells(getSelectionCells(), true);
		}
			...
			if (!tmp.isEmpty())
			{
				...
				for (int i = 0; i < cells.length; i++)
				{
					...
					else
					{
						...
						if (g != null)
						{
							JGraphXCellState state = view.getState(cells[i]);
							JGraphXCellState pstate = view
			...
			Object source, Object target, boolean absolute, boolean constrain)
	{
		if (cells != null && parent != null && index != null)
		{
			...
			try
			{
				JGraphXCellState parentState = (absolute) ? view.getState(parent)
						: null;
				...
				for (int i = 0; i < cells.length; i++)
				{
					...
					else
					{
						...
						if (o1 != null && cells[i] != parent
								&& parent != previous)
						{
							JGraphXCellState oldState = view.getState(previous);
				...
				for (int i = 0; i < cells.length; i++)
				{
					...
					for (int j = 0; j < edges.length; j++)
					{
						if (!cellSet.contains(edges[j]))
						{
							...
							if (geo != null)
							{
								JGraphXCellState state = view.getState(edges[j]);
			...
			try
			{
				...
				if (size != null && geo != null)
				{
					...
					if (isSwimlane(cell))
					{
						...
						if (JGraphXUtils.isTrue(style, mxConstants.STYLE_HORIZONTAL,
								true))
						{
	...
	public mxRectangle getPreferredSizeForCell(Object cell)
	{
		...
		if (cell != null)
		{
			...
			if (style != null && !model.isEdge(cell))
			{
				...
				if (getImage(state) != null || JGraphXUtils.getString(style,
						mxConstants.STYLE_IMAGE) != null)
				{
					if (JGraphXUtils.getString(style, mxConstants.STYLE_SHAPE, "")
							.equals(mxConstants.SHAPE_LABEL))
					{
						...
						{
							dx += JGraphXUtils.getDouble(style,
						...
						{
							dy += JGraphXUtils.getDouble(style,
				...
				// Adds spacings
				double spacing = JGraphXUtils.getDouble(style,
						mxConstants.STYLE_SPACING);
				dx += 2 * spacing;
				dx += JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING_LEFT);
				dx += JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING_RIGHT);

				dy += 2 * spacing;
				dy += JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING_TOP);
				dy += JGraphXUtils.getDouble(style,
				...
				{
					mxRectangle size = JGraphXUtils.getLabelSize(value, style,
							isHtmlLabel(cell), 1);
	 ...
	 */
	public ConnectionConstraint getConnectionConstraint(JGraphXCellState edge,
														JGraphXCellState terminal, boolean source)
	{
		mxPoint point = null;
		...
		if (point != null)
		{
			perimeter = JGraphXUtils.isTrue(edge.style,
	 ...
	 */
	public mxPoint getConnectionPoint(JGraphXCellState vertex,
                                      ConnectionConstraint constraint)
	{
		...
		if (vertex != null && constraint.point != null)
		{
			...
			String direction = JGraphXUtils.getString(vertex.getStyle(),
			...
			double r2 = JGraphXUtils.getDouble(vertex.getStyle(),
			...
			if (constraint.perimeter)
			{
				if (r1 != 0)
				{
					...
					point = JGraphXUtils.getRotatedPoint(point, cos, sin, cx);
				}
			...
			else
			{
				...
				{
					boolean flipH = JGraphXUtils.getString(vertex.getStyle(),
							mxConstants.STYLE_FLIPH).equals(1);
					boolean flipV = JGraphXUtils.getString(vertex.getStyle(),
	...
	public void exitGroup()
	{
		...
		if (current != null)
		{
			...
			JGraphXCellState state = view.getState(current);
	 ...
	 * @param edge Cell state that represents the edge.
	 */
	public boolean isOrthogonal(JGraphXCellState edge)
	{
		if (edge.getStyle().containsKey(mxConstants.STYLE_ORTHOGONAL))
		{
			return JGraphXUtils.isTrue(edge.getStyle(),
	 ...
	 */
	public boolean isLoop(JGraphXCellState state)
	{
	...
	public String getCellValidationError(Object cell)
	{
		...
		if (multiplicities != null)
		{
			for (int i = 0; i < multiplicities.length; i++)
			{
				...
				if (rule.source
						&& JGraphXUtils.isNode(value, rule.type, rule.attr,
								rule.value)
						&& ((max == 0 && outCount > 0)
	...
	public String getLabel(Object cell)
	{
		...
		if (cell != null)
		{
			...
			if (labelsVisible
					&& !JGraphXUtils.isTrue(style, mxConstants.STYLE_NOLABEL, false))
			{
	...
	public mxRectangle getStartSize(Object swimlane)
	{
		...
		JGraphXCellState state = view.getState(swimlane);
		Map<String, Object> style = (state != null) ? state.getStyle()
				: getCellStyle(swimlane);

		if (style != null)
		{
			double size = JGraphXUtils.getDouble(style, mxConstants.STYLE_STARTSIZE,
	 ...
	 */
	public String getImage(JGraphXCellState state)
	{
		return (state != null && state.getStyle() != null)
				? JGraphXUtils.getString(state.getStyle(), mxConstants.STYLE_IMAGE)
				: null;
	}
	...
	public boolean isSwimlane(Object cell)
	{
		if (cell != null)
		{
			if (model.getParent(cell) != model.getRoot())
			{
				...
				if (style != null && !model.isEdge(cell))
				{
					return JGraphXUtils.getString(style, mxConstants.STYLE_SHAPE, "")
	...
	public boolean isCellEditable(Object cell)
	{
		...
		return isCellsEditable() && !isCellLocked(cell)
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_EDITABLE, true);
	}
	...
	public boolean isCellResizable(Object cell)
	{
		...
		return isCellsResizable() && !isCellLocked(cell)
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_RESIZABLE, true);
	}
	...
	public boolean isCellMovable(Object cell)
	{
		...
		return isCellsMovable() && !isCellLocked(cell)
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_MOVABLE, true);
	}
	...
	public boolean isCellBendable(Object cell)
	{
		...
		return isCellsBendable() && !isCellLocked(cell)
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_BENDABLE, true);
	}
	...
	public boolean isCellDeletable(Object cell)
	{
		...
		return isCellsDeletable()
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_DELETABLE, true);
	}
	...
	public boolean isCellCloneable(Object cell)
	{
		...
		return isCellsCloneable()
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_CLONEABLE, true);
	}
	...
	public boolean isLabelClipped(Object cell)
	{
		if (!isLabelsClipped())
		{
			...
			return (style != null)
					? JGraphXUtils.getString(style, mxConstants.STYLE_OVERFLOW, "")
							.equals("hidden")
					: false;
		}
	...
	public boolean isAutoSizeCell(Object cell)
	{
		...
		return isAutoSizeCells()
				|| JGraphXUtils.isTrue(style, mxConstants.STYLE_AUTOSIZE, false);
	}
	...
	public boolean isCellFoldable(Object cell, boolean collapse)
	{
		...
		return model.getChildCount(cell) > 0
				&& JGraphXUtils.isTrue(style, mxConstants.STYLE_FOLDABLE, true);
	}
	...
	public Object getDropTarget(Object[] cells, Point pt, Object cell)
	{
		...
		return (model.getParent(cell) != model.getRoot()
				&& !JGraphXUtils.contains(cells, cell)) ? cell : null;
	};
			...
			boolean sources, boolean targets)
	{
		...
		if (edges != null)
		{
			...
			{
				JGraphXCellState state = view.getState(edges[i]);
				Object source = (state != null) ? state.getVisibleTerminal(true)
		...
		if (rightHalfpane || bottomHalfpane)
		{
			...
			if (parent != null)
			{
				for (int i = 0; i < childCount; i++)
				{
					...
					JGraphXCellState state = view.getState(child);
	 ...
	 */
	public void drawGraph(IJGraphXCanvas canvas)
	{
	 ...
	 */
	public void drawCell(IJGraphXCanvas canvas, Object cell)
	{
						  ...
						  boolean drawLabel)
	{
				...
				&& (model.isVertex(cell) || model.isEdge(cell)))
		{
			...
			IJGraphXCanvas clippedCanvas = (isLabelClipped(state.getCell())) ? canvas
					: null;

			if (clippedCanvas instanceof JGraphXImageCanvas)
			{
				clippedCanvas = ((JGraphXImageCanvas) clippedCanvas)
						.getGraphicsCanvas();
	 ...
	 */
	protected void cellDrawn(IJGraphXCanvas canvas, JGraphXCellState state,
