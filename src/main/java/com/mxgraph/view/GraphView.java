...
import com.mxgraph.view.EdgeStyle.mxEdgeStyleFunction;
import com.mxgraph.view.Perimeter.mxPerimeterFunction;
...
public class GraphView extends mxEventSource
{
	 ...
	 */
	protected Hashtable<Object, JGraphXCellState> states = new Hashtable<Object, JGraphXCellState>();
	 ...
	 *            Reference to the enclosing graph.
	 */
	public GraphView(JGraphX graph)
	{
		this.graph = graph;
	}
	 ...
	 */
	public Hashtable<Object, JGraphXCellState> getStates()
	{
		return states;
	}
	 ...
	 * Returns the dictionary that maps from cells to states.
	 */
	public void setStates(Hashtable<Object, JGraphXCellState> states)
	{
		this.states = states;
	}
	...
	public mxRectangle getBounds(Object[] cells, boolean boundingBox)
	{
		...
		if (cells != null && cells.length > 0)
		{
			...
			for (int i = 0; i < cells.length; i++)
			{
				...
				{
					JGraphXCellState state = getState(cells[i]);
	 ...
	 */
	public mxRectangle getBoundingBox(JGraphXCellState state)
	{
	 ...
	 */
	public mxRectangle getBoundingBox(JGraphXCellState state, boolean recurse)
	{
	...
	public Object validateCell(Object cell, boolean visible)
	{
		if (cell != null)
		{
			...
			JGraphXCellState state = getState(cell, visible);
	 ...
	 */
	public JGraphXCellState validateCellState(Object cell)
	{
	 ...
	 *            validated.
	 */
	public JGraphXCellState validateCellState(Object cell, boolean recurse)
	{
	 ...
	 */
	public void updateCellState(JGraphXCellState state)
	{
	 ...
	 */
	public void updateVertexLabelOffset(JGraphXCellState state)
	{
	 ...
	 */
	public void updateLabel(JGraphXCellState state)
	{
		...
		if (label != null
				...
				&& JGraphXUtils.getString(style, mxConstants.STYLE_WHITE_SPACE,
						"nowrap").equals("wrap"))
		{
			...
			String[] lines = JGraphXUtils.wordWrap(label,
					JGraphXUtils.getFontMetrics(JGraphXUtils.getFont(state.getStyle())),
	 ...
	 */
	public double getWordWrapWidth(JGraphXCellState state)
	{
		Map<String, Object> style = state.getStyle();
		...
		if (horizontal)
		{
			w = (state.getWidth() / scale) - 2 * mxConstants.LABEL_INSET - 2
					* JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING)
					- JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING_LEFT)
					- JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING_RIGHT);
		}
		else
		{
			w = (state.getHeight() / scale)
					...
					* mxConstants.LABEL_INSET
					- 2
					* JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING)
					- JGraphXUtils.getDouble(style, mxConstants.STYLE_SPACING_TOP)
					+ JGraphXUtils
	 ...
	 */
	public void updateLabelBounds(JGraphXCellState state)
	{
		...
		else if (state.getLabel() != null)
		{
			...
			state.setLabelBounds(JGraphXUtils.getLabelPaintBounds(state.getLabel(),
	 ...
	 */
	public mxRectangle updateBoundingBox(JGraphXCellState state)
	{
		...
		double strokeWidth = Math.max(
				1,
				Math.round(JGraphXUtils.getInt(style, mxConstants.STYLE_STROKEWIDTH,
						1) * scale));
		...
		if (graph.getModel().isEdge(state.getCell()))
		{
			...
			if (JGraphXUtils.getString(style, mxConstants.STYLE_SHAPE, "").equals(
		...
		if (JGraphXUtils.isTrue(style, mxConstants.STYLE_SHADOW))
		{
		...
		{
			if (JGraphXUtils.getString(style, mxConstants.STYLE_IMAGE) != null)
			{
				double w = JGraphXUtils.getInt(style, mxConstants.STYLE_IMAGE_WIDTH,
						mxConstants.DEFAULT_IMAGESIZE) * scale;
				double h = JGraphXUtils.getInt(style,
		...
		mxRectangle bbox = JGraphXUtils.getBoundingBox(rect, rotation);
	 ...
	 */
	public void updateFixedTerminalPoints(JGraphXCellState edge, JGraphXCellState source,
                                          JGraphXCellState target)
	{
	 ...
	 *            Cell state whose initial terminal points should be updated.
	 */
	public void updateFixedTerminalPoint(JGraphXCellState edge,
                                         JGraphXCellState terminal, boolean source,
	 ...
	 */
	public void updatePoints(JGraphXCellState edge, List<mxPoint> points,
                             JGraphXCellState source, JGraphXCellState target)
	{
		if (edge != null)
		{
			...
			if (edgeStyle != null)
			{
				JGraphXCellState src = getTerminalPort(edge, source, true);
				JGraphXCellState trg = getTerminalPort(edge, target, false);
	 ...
	 */
	public mxPoint transformControlPoint(JGraphXCellState state, mxPoint pt)
	{
		mxPoint origin = state.getOrigin();
	 ...
	 */
	public mxEdgeStyleFunction getEdgeStyle(JGraphXCellState edge,
                                            List<mxPoint> points, Object source, Object target)
	{
		...
		else if (!JGraphXUtils.isTrue(edge.getStyle(),
	 ...
	 *            Cell state that represents the target terminal.
	 */
	public void updateFloatingTerminalPoints(JGraphXCellState state,
                                             JGraphXCellState source, JGraphXCellState target)
	{
	...
	public void updateFloatingTerminalPoint(JGraphXCellState edge,
                                            JGraphXCellState start, JGraphXCellState end, boolean source)
	{
		start = getTerminalPort(edge, start, source);
		mxPoint next = getNextPoint(edge, end, source);
		double border = JGraphXUtils.getDouble(edge.getStyle(),
				mxConstants.STYLE_PERIMETER_SPACING);
		border += JGraphXUtils.getDouble(edge.getStyle(),
	 ...
	 */
	public JGraphXCellState getTerminalPort(JGraphXCellState state, JGraphXCellState terminal,
                                            boolean source)
	{
	 ...
	 * the given point.
	 */
	public mxPoint getPerimeterPoint(JGraphXCellState terminal, mxPoint next,
                                     boolean orthogonal)
	{
	 ...
	 */
	public double getRoutingCenterX(JGraphXCellState state)
	{
		float f = (state.getStyle() != null) ? JGraphXUtils.getFloat(
	 ...
	 */
	public double getRoutingCenterY(JGraphXCellState state)
	{
		float f = (state.getStyle() != null) ? JGraphXUtils.getFloat(
	 ...
	 */
	public mxRectangle getPerimeterBounds(JGraphXCellState terminal, double border)
	{
		if (terminal != null)
		{
			border += JGraphXUtils.getDouble(terminal.getStyle(),
					mxConstants.STYLE_PERIMETER_SPACING);
		}
	 ...
	 */
	public mxPerimeterFunction getPerimeterFunction(JGraphXCellState state)
	{
	 ...
	 */
	public mxPoint getNextPoint(JGraphXCellState edge, JGraphXCellState opposite,
                                boolean source)
	{
	 ...
	 */
	public void updateEdgeBounds(JGraphXCellState state)
	{
	 ...
	 */
	public mxPoint getPoint(JGraphXCellState state)
	{
	 ...
	 */
	public JGraphXCellState[] getCellStates(Object[] cells)
	{
		List<JGraphXCellState> result = new ArrayList<JGraphXCellState>(cells.length);
		...
		JGraphXCellState[] resultArray = new JGraphXCellState[result.size()];
	 ...
	 */
	public JGraphXCellState getState(Object cell)
	{
	 ...
	 * @return Returns the state for the given cell.
	 */
	public JGraphXCellState getState(Object cell, boolean create)
	{
		JGraphXCellState state = null;
	 ...
	 */
	public JGraphXCellState removeState(Object cell)
	{
		return (cell != null) ? (JGraphXCellState) states.remove(cell) : null;
	}
	 ...
	 */
	public JGraphXCellState createState(Object cell)
	{
		return new JGraphXCellState(this, cell, graph.getCellStyle(cell));
	}
