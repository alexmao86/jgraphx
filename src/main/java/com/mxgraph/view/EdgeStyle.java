...
public class EdgeStyle
{
	...
	public interface mxEdgeStyleFunction
	{
		 ...
		 * edge.
		 */
		void apply(JGraphXCellState state, JGraphXCellState source, JGraphXCellState target,
	...
	public static mxEdgeStyleFunction EntityRelation = new mxEdgeStyleFunction()
	{
		 ...
		 */
		public void apply(JGraphXCellState state, JGraphXCellState source,
                          JGraphXCellState target, List<mxPoint> points, List<mxPoint> result)
		{
			...
			if (p0 != null)
			{
				source = new JGraphXCellState();
				source.setX(p0.getX());
				source.setY(p0.getY());
			}
			else if (source != null)
			{
				int constraint = JGraphXUtils.getPortConstraints(source, state, true, mxConstants.DIRECTION_MASK_NONE);
			...
			if (pe != null)
			{
				target = new JGraphXCellState();
				target.setX(pe.getX());
				target.setY(pe.getY());
			}
			else if (target != null)
			{
				int constraint = JGraphXUtils.getPortConstraints(target, state, false, mxConstants.DIRECTION_MASK_NONE);
	...
	public static mxEdgeStyleFunction Loop = new mxEdgeStyleFunction()
	{
                          ...
                          JGraphXCellState target, List<mxPoint> points, List<mxPoint> result)
		{
			if (source != null)
			{
				...
				double seg = JGraphXUtils.getDouble(state.getStyle(),
						mxConstants.STYLE_SEGMENT, graph.getGridSize())
						* view.getScale();
				String dir = JGraphXUtils
	...
	public static mxEdgeStyleFunction ElbowConnector = new mxEdgeStyleFunction()
	{
                          ...
                          JGraphXCellState target, List<mxPoint> points, List<mxPoint> result)
		{
			...
			if (!horizontal
					&& (vertical || JGraphXUtils.getString(state.getStyle(),
	...
	public static mxEdgeStyleFunction SegmentConnector = new mxEdgeStyleFunction()
	{
		 ...
		 * @see com.mxgraph.view.mxEdgeStyle.mxEdgeStyleFunction#apply(com.mxgraph.view.mxCellState, com.mxgraph.view.mxCellState, com.mxgraph.view.mxCellState, java.util.List, java.util.List)
		 */
		public void apply(JGraphXCellState state, JGraphXCellState source, JGraphXCellState target, List<mxPoint> hints, List<mxPoint> result)
		{
			...
			if (hints != null && hints.size() > 0)
			{
				...
				JGraphXCellState currentTerm = source;
	...
	public static mxEdgeStyleFunction OrthConnector = new mxEdgeStyleFunction()
	{
		...
		public void apply(JGraphXCellState state, JGraphXCellState source,
                          JGraphXCellState target, List<mxPoint> points, List<mxPoint> result)
		{
			JGraphX graph = state.view.graph;
			...
			if (source != null && target != null)
			{
				...
				portConstraint[0] = JGraphXUtils.getPortConstraints(source, state,
						true);
				portConstraint[1] = JGraphXUtils.getPortConstraints(target, state,
						false);
				...
				horPref[1] = JGraphXUtils.reversePortConstraints(horPref[0]);
				vertPref[1] = JGraphXUtils.reversePortConstraints(vertPref[0]);
				...
				for (int i = 0; i < 2; i++)
				{
					...
					{
						horPref[i] = JGraphXUtils.reversePortConstraints(horPref[i]);
					}

					if ((vertPref[i] & portConstraint[i]) == 0)
					{
						vertPref[i] = JGraphXUtils
