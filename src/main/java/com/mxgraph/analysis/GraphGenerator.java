...
import com.mxgraph.generatorfunction.GeneratorFunction;
import com.mxgraph.generatorfunction.GeneratorRandomFunction;
...
public class GraphGenerator
{
	 ...
	 */
	public static GeneratorFunction getGeneratorFunction(JGraphX graph, boolean weighted, double minWeight, double maxWeight)
	{
		if (weighted)
		{
	...
	public void getKnightTour(AnalysisGraph aGraph, int xDim, int yDim, int startVertexValue) throws StructuralException
	{
		...
		while (nextMove != null)
		{
			...
			currValue = (int) costFunction.getCost(new JGraphXCellState(view, nextMove, null));
	...
	private Object getNextKnightMove(AnalysisGraph aGraph, int xDim, int yDim, int xCoord, int yCoord, ArrayList<Object> resultPath)
	{
		...
		{
			int currValue = (int) costFunction.getCost(new JGraphXCellState(view, possibleMoves[i], null));
