...
public class GraphStructure
{
	...
	public static Object getVertexWithValue(AnalysisGraph aGraph, int value)
	{
		...
		for (int i = 0; i < childNum; i++)
		{
			...
			vertexValue = (int) costFunction.getCost(new JGraphXCellState(view, currVertex, null));
	...
	public static boolean isCutVertex(AnalysisGraph aGraph, Object vertex)
	{
		...
		if (aGraph.getEdges(vertex, null, true, true, false, true).length >= 2)
		{
			...
			Object newVertex = getVertexWithValue(aGraphCopy,
					(int) aGraph.getGenerator().getCostFunction().getCost(new JGraphXCellState(graph.getView(), vertex, null)));
	...
	public static boolean isCutEdge(AnalysisGraph aGraph, Object edge)
	{
		...
		int srcValue = (int) costFunction.getCost(new JGraphXCellState(view, aGraph.getTerminal(edge, true), null));
		int destValue = (int) costFunction.getCost(new JGraphXCellState(view, aGraph.getTerminal(edge, false), null));
		...
		if (aGraph.getTerminal(edge, false) != null || aGraph.getTerminal(edge, true) != null)
		{
			...
			int currSrcValue = (int) costFunctionCopy.getCost(new JGraphXCellState(viewCopy, aGraphCopy.getTerminal(currEdge, true), null));
			int currDestValue = (int) costFunctionCopy.getCost(new JGraphXCellState(viewCopy, aGraphCopy.getTerminal(currEdge, false), null));
			int i = 0;
