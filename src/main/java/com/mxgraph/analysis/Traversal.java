		...
		while (vertexList.size() > 0)
		{
			...
			for (int j = 0; j < neighborVertices.length; j++)
			{
				...
				if (vertexList.contains(currNeighbor))
				{
					...
					currEdgeWeight = costFunction.getCost(new JGraphXCellState(view, connectingEdge, null));
	...
	public static List<Map<Object, Object>> bellmanFord(AnalysisGraph aGraph, Object startVertex) throws StructuralException
	{
		...
		for (int i = 0; i < vertexNum; i++)
		{
			for (int j = 0; j < edgeNum; j++)
			{
				...
				double dist = (Double) distanceMap.get(source) + costFunction.getCost(new JGraphXCellState(view, currEdge, null));
				...
				{
					dist = (Double) distanceMap.get(target) + costFunction.getCost(new JGraphXCellState(view, currEdge, null));
